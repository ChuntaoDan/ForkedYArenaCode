...
% odors to deliver
try
    load('C:\Users\rajagopalana\Documents\MATLAB\Y-Arena\binary_masks.mat')
catch
    ...
    save('C:\Users\rajagopalana\Documents\MATLAB\Y-Arena\binary_masks.mat','binarymask1','binarymask2','binarymask3','binarymask4','binarymask5','binarymask6','binarymask7','binarymask8','binarymask9'); 
end
    ...
    elseif od_state == 1
        if PresentZone ~= find([binarymask1(xy_now(2),xy_now(1)),binarymask2(xy_now(2),xy_now(1)),binarymask3(xy_now(2),xy_now(1)),binarymask4(xy_now(2),xy_now(1)),binarymask5(xy_now(2),xy_now(1)),binarymask6(xy_now(2),xy_now(1)),binarymask7(xy_now(2),xy_now(1)),binarymask8(xy_now(2),xy_now(1)),binarymask9(xy_now(2),xy_now(1))]==1,1)
            ...
            if PresentZone == 6 && PastZone == 3
                if ch_state == 1
                    servos.exposeForDuration(2,10) % 10 second exposure for OCT
                    pause(10)
                    od_state = 0
                elseif ch_state == 2
%                     servos.exposeForDuration(2,0) % 0 second exposure for MCH
%                     pause(0)
                    od_state = 0
                end
            elseif PresentZone == 5 && PastZone == 2
                if ch_state == 1
%                     servos.exposeForDuration(1,0) % 0 second exposure for MCH
%                     pause(0)
                    od_state = 0
                elseif ch_state == 2
                    servos.exposeForDuration(1,10) % 10 second exposure for OCT
                    pause(10)
                    od_state = 0
                end  
            end
...
%                     pause(0)
            ...
            elseif PresentZone == 4 && PastZone == 1
                if ch_state == 1
                    servos.exposeForDuration(0,10) % 10 second exposure for OCT
                    pause(10)
                    od_state = 0
                elseif ch_state == 2
%                     servos.exposeForDuration(0,0) % 0 second exposure for MCH
%                     pause(0)
                    od_state = 0
                end  
            end
