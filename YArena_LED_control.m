...
switch upper(strtrim(token))
    ...
    case 'RED'
        Red_int_val = round(param);   % this is done so only one dec place
        
        %send command to controller
        fprintf(s1, ['RED ',num2str(Red_int_val)]);
        if docheckstatus, [~,status] = checkControllerStatus(s1,dispstatus); end
        
    case 'GREEN'
        Green_int_val = round(param);   % this is done so only one dec place
        
        %send command to controller
        fprintf(s1, ['GREEN ',num2str(Green_int_val)]);
        if docheckstatus, [~,status] = checkControllerStatus(s1,dispstatus); end    
    
    case 'BLUE'
        Blue_int_val = round(param);   % this is done so only one dec place
        
        %send command to controller
        fprintf(s1, ['BLU ',num2str(Blue_int_val)]);
        if docheckstatus, [~,status] = checkControllerStatus(s1,dispstatus); end
    ...
    case 'PULSE'
        fprintf(s1, ['PULSE,', num2str(param.pulse_width),',',num2str(param.pulse_period),',',num2str(param.number_of_pulses), ',', ...
            num2str(param.pulse_train_interval),',',num2str(param.LED_delay),',',num2str(param.iteration)]);
        if docheckstatus, [~,status] = checkControllerStatus(s1,dispstatus); end
