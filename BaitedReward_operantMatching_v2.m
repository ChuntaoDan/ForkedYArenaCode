...
dataPath = [tempPath1, '\', handles.expStartTime, '_',handles.rig, '_',...
    'Cam', num2str(1-1),'BR_V2_64f_Fly20'];
...
%% RUN EXPT
% THIS VERSION OF THE EXPT SHOULD CORRECTLY COLLECT REWARD TIMING
% INFORMATION
% A SINGLE FLY IN THIS VERSION EXPERIENCES 3 CONTINGENCIES /BLOCKS OF 80
% TRIALS EACH WITH RANDOM (PRE-DEFINED BY USED) CONTINGENCIES OF REWARD
...
y = input('Enter intial reward probability for OCT - Range 0 to 1')
z = input('Enter intial reward probability for OCT - Range 0 to 1')
totalp = input('Enter sum of both probabilities (OCT + MCH) - 0.5 OR 1')
time = 0;


    while n_trials < 241 && time<14400
        if exist('vidobj2') == 0
            vidobj2 = videoinput('pointgrey',2);
            triggerconfig(vidobj2, 'manual');
            vidobj2.FramesPerTrigger = inf; 
        ...
        tic
        if n_trials == 1
            [n_trials,time_n,vidobj2] = run_section_expt(n_trials,dataPath,x,ave_background,binarymask1,binarymask2,binarymask3,binarymask4,binarymask5,binarymask6,binarymask7,binarymask8,binarymask9,valvedio1,valvedio2,valvedio3,hComm,vidobj2,totalp)
            time = time+time_n;
        elseif n_trials == 81
            [n_trials,time_n,vidobj2] = run_section_expt(n_trials,dataPath,y,ave_background,binarymask1,binarymask2,binarymask3,binarymask4,binarymask5,binarymask6,binarymask7,binarymask8,binarymask9,valvedio1,valvedio2,valvedio3,hComm,vidobj2,totalp)
            time = time+time_n;
        elseif n_trials == 161
            [n_trials,time_n,vidobj2] = run_section_expt(n_trials,dataPath,z,ave_background,binarymask1,binarymask2,binarymask3,binarymask4,binarymask5,binarymask6,binarymask7,binarymask8,binarymask9,valvedio1,valvedio2,valvedio3,hComm,vidobj2,totalp)
            time = time+time_n;
        end
