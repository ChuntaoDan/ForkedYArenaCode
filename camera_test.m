...
dataPath = [tempPath1, '\', handles.expStartTime, '_',handles.rig, '_',...
    'Cam', num2str(1-1),'ACV_protocol2_4'];
...
% OLD CODE FOR SAVING VIDEO DIRECTLY VIA MATLAB

% for i = 1:2
% % Defining features of the video object that will be recorded
%     vidobj1 = videoinput('pointgrey',1);
%     src = getselectedsource(vidobj1);
%     src.FrameRatePercentage = 50;
%     vidobj1.FramesPerTrigger = Inf;
%     % vidobj.TriggerRepeat = 50;
%     vidobj1.LoggingMode = 'disk';
%     filename = sprintf('logfile%d.avi',i);
%     logfile = VideoWriter(filename);
%     vidobj1.DiskLogger = logfile;
%     preview(vidobj1);
%     start(vidobj1); % start recording
%    

% When delivering ACV down just one of the arms uncomment the line below
% state = [0 0 0 1 0 1];
% s=FlipValveUSB6525({'Vial1','Vial2','Vial3','Vial4','Vial5','Final'},state,valvedio1); % ACV OPEN (VIAL 5)
...
%     if time > 180 && time <300 && sum(state == [0 0 0 0 1 1])~=6
%         state = [0 0 0 0 1 1];
%         s=FlipValveUSB6525({'Vial1','Vial2','Vial3','Vial4','Vial5','Final'},[0 0 0 0 1 1],valvedio); % ACV OPEN (VIAL 5)
%     elseif time > 480 && time <600 && sum(state == [0 0 0 0 1 1])~=6
%         state = [0 0 0 0 1 1];
%         s=FlipValveUSB6525({'Vial1','Vial2','Vial3','Vial4','Vial5','Final'},[0 0 0 0 1 1],valvedio); % ACV OPEN (VIAL 5)
%     elseif time > 780 && time <900 && sum(state == [0 0 0 0 1 1])~=6
%         state = [0 0 0 0 1 1];
%         s=FlipValveUSB6525({'Vial1','Vial2','Vial3','Vial4','Vial5','Final'},[0 0 0 0 1 1],valvedio); % ACV OPEN (VIAL 5)
%     elseif time > 1080 && time <1200 && sum(state == [0 0 0 0 1 1])~=6
%         state = [0 0 0 0 1 1];
%         s=FlipValveUSB6525({'Vial1','Vial2','Vial3','Vial4','Vial5','Final'},[0 0 0 0 1 1],valvedio); % ACV OPEN (VIAL 5)
%     elseif time > 1380 && time <1500 && sum(state == [0 0 0 0 1 1])~=6
%         state = [0 0 0 0 1 1];
%         s=FlipValveUSB6525({'Vial1','Vial2','Vial3','Vial4','Vial5','Final'},[0 0 0 0 1 1],valvedio); % ACV OPEN (VIAL 5)
%     elseif time > 1680 && time <1800 && sum(state == [0 0 0 0 1 1])~=6
%         state = [0 0 0 0 1 1];
%         s=FlipValveUSB6525({'Vial1','Vial2','Vial3','Vial4','Vial5','Final'},[0 0 0 0 1 1],valvedio); % ACV OPEN (VIAL 5)
%     elseif time <180 && sum(state == [0 0 0 0 1 1]) == 6
%         state = [0 0 0 1 0 1];
%         s=FlipValveUSB6525({'Vial1','Vial2','Vial3','Vial4','Vial5','Final'},state,valvedio); % ACV OPEN (VIAL 5)
%     elseif time <480 && time > 300 && sum(state == [0 0 0 0 1 1]) == 6
%         state = [0 0 0 1 0 1];
%         s=FlipValveUSB6525({'Vial1','Vial2','Vial3','Vial4','Vial5','Final'},state,valvedio); % ACV OPEN (VIAL 5)
%     elseif time <780 && time > 600 && sum(state == [0 0 0 0 1 1]) == 6
%         state = [0 0 0 1 0 1];
%         s=FlipValveUSB6525({'Vial1','Vial2','Vial3','Vial4','Vial5','Final'},state,valvedio); % ACV OPEN (VIAL 5)
%     elseif time <1080 && time > 900 && sum(state == [0 0 0 0 1 1]) == 6
%         state = [0 0 0 1 0 1];
%         s=FlipValveUSB6525({'Vial1','Vial2','Vial3','Vial4','Vial5','Final'},state,valvedio); % ACV OPEN (VIAL 5)
%     elseif time <1380 && time > 1200 && sum(state == [0 0 0 0 1 1]) == 6
%         state = [0 0 0 1 0 1];
%         s=FlipValveUSB6525({'Vial1','Vial2','Vial3','Vial4','Vial5','Final'},state,valvedio); % ACV OPEN (VIAL 5)
%     elseif time <1680 && time > 1500 && sum(state == [0 0 0 0 1 1]) == 6
%         state = [0 0 0 1 0 1];
%         s=FlipValveUSB6525({'Vial1','Vial2','Vial3','Vial4','Vial5','Final'},state,valvedio); % ACV OPEN (VIAL 5)
% 
%     end
