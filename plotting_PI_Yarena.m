...
individual_pi = zeros(22501,9,length(expts));
        ...
        if ~isempty(OCT) %OCT
            ...
            for trial_n = 1:length(trials)
                % Create indicator for arena number
                 ...
                 if ~isempty(strfind(trials(trial_n).name,'arm0')) 
                     allflycounts_OCT( 1:length(n_flies),count, 1) = n_flies(1:length(n_flies), 1);
                     allflycounts_OCT( 1:length(n_flies),count, 2) = n_flies(1:length(n_flies), 2) + n_flies(1:length(n_flies), 3);
                 elseif  ~isempty(strfind(trials(trial_n).name,'arm1')) 
                     allflycounts_OCT( 1:length(n_flies),count, 1) = n_flies(1:length(n_flies), 2);
                     allflycounts_OCT( 1:length(n_flies),count, 2) = n_flies(1:length(n_flies), 1) + n_flies(1:length(n_flies), 3);
                 else
                     allflycounts_OCT( 1:length(n_flies),count, 1) = n_flies(1:length(n_flies), 3);
                     allflycounts_OCT( 1:length(n_flies),count, 2) = n_flies(1:length(n_flies), 1) + n_flies(1:length(n_flies), 2);
                 end
            ...
            for trial_n = 1:length(trials)
                % Create indicator for arena number
                 ...
                 if ~isempty(strfind(trials(trial_n).name,'arm0')) 
                     ...
                     allflycounts_MCH( 1:length(n_flies),count, 2) = n_flies(1:length(n_flies), 2) + n_flies(1:length(n_flies), 3);
                 elseif  ~isempty(strfind(trials(trial_n).name,'arm1')) 
                     allflycounts_MCH( 1:length(n_flies),count, 1) = n_flies(1:length(n_flies), 2);
                     allflycounts_MCH( 1:length(n_flies),count, 2) = n_flies(1:length(n_flies), 1) + n_flies(1:length(n_flies), 3);
                 else
                     allflycounts_MCH( 1:length(n_flies),count, 1) = n_flies(1:length(n_flies), 3);
                     allflycounts_MCH( 1:length(n_flies),count, 2) = n_flies(1:length(n_flies), 1) + n_flies(1:length(n_flies), 2);
                 end
                 cd(cond)
...
%     end    
    ...
    OCT_pi = (mean((allflycounts_OCT(:,1:count,1)-(1/2)*allflycounts_OCT(:,1:count,2)),2)./mean((allflycounts_OCT(:,1:count,1)+(1/2)*allflycounts_OCT(:,1:count,2)),2));
    OCT_se = 1/sqrt(trial_n-2).*(std(transpose(((allflycounts_OCT(:,1:count,1)-(1/2)*allflycounts_OCT(:,1:count,2)))./((allflycounts_OCT(:,1:count,1)+(1/2)*allflycounts_OCT(:,1:count,2))))));
    
    % Average the paired & unpaired odor trials for the second camera
    MCH_pi = (mean(allflycounts_MCH(:,1:count,1)-(1/2)*allflycounts_MCH(:,1:count,2),2)./mean(allflycounts_MCH(:,1:count,1)+(1/2)*allflycounts_MCH(:,1:count,2),2));
    MCH_se = 1/sqrt(trial_n-2).*(std(transpose((allflycounts_MCH(:,1:count,1)-(1/2)*allflycounts_MCH(:,1:count,2))./(allflycounts_MCH(:,1:count,1)+(1/2)*allflycounts_MCH(:,1:count,2)))));
