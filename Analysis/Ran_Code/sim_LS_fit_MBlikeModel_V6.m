%% Simulating multiple cov-rule go no go datasets and fitting wit MBlike model
...
C1_1000 = zeros(300,1); % variable of size k,1 containing the number of times option 1 was chosen to calculate CR
C2_1000 = zeros(300,1); % variable of size k,1 containing the number of times option 2 was chosen to calculate CR
In1 = zeros(300,1); % variable of size k,1 containing the number of times option 1 was rewarded to calculate IR
In2 = zeros(300,1); % variable of size k,1 containing the number of times option 2 was rewarded to calculate RR
y_vec = {};
R_vec = {};
W_vec = {};
S_vec = {};
...
%     k
    ...
    p1 = 0.3*rand(1,1); % Probability of reward for odor 1
    p2 = 0.3-p1; % Probability of reward for odor 2
    % IF YOU WANT POTENTIATION LEARNING RULE UNCOMMENT THE FOLLOWING LINE
    [In1(k),In2(k),C1_1000(k),C2_1000(k),y_vec{k},R_vec{k},W_vec{k},S_vec{k},y_cont{k}] = Loewenstein_Seung_v7_JEF(p1,p2,noisy_inputs,alpha,c,sigma2); % simulate the behavior
...
for k = 1:50
    ...
    X = squeeze(S_vec(2:length(S_vec),:,k)); % stimulus history on all trials including turn away trials
...
%     R(:,2) = R(:,1); % Here the reward vector is duplicated. 
    ...
    for timept = 2:length(Y)
        ...
        % Here the average reward for option 2 is computed over the last 10
        % exposures to option 2.
...
% Plotting fit timecourses
    ...
    filename = sprintf('fit_timecourse_sim_SDREsingleW_model_SDRE_drule_%d.fig',k);
