function [I1,I2,C1_1000,C2_1000,y,R_vec,W_vec,S_vec,y_cont] = Loewenstein_Seung_v7_JEF_multBlock(p1_mat,p2_mat)
...
for trialn = 1:300 % The number of trials is hardcoded as 2000
    if trialn <= 100
        p1 = p1_mat(1);
        p2 = p2_mat(1);
    elseif trialn > 100 && trialn <= 200
        p1 = p1_mat(2);
        p2 = p2_mat(2);
    else
        p1 = p1_mat(3);
...
%     
    %     % update rule for cov LR where both options are modified
%     if cS1 == 1
%         dW1 = n * (R_vec(end,1)-eR1) * (cS1); % This expected reward is stimulus specific, whereas the simulation is usually not
%         dW2 = n * (R_vec(end,2)-eR2) * (cS1);
%         W1 = W1 + transpose(dW1);
%         W2 = W2 + transpose(dW2);
%         % Why put a floor on the weights but not a ceiling. 
% %         if W1 < 0 % This nonlinear puts a floor on the weights. This nonlinearity is not assumed in the fitting. 
...
%         W_vec(length(W_vec(:,1)),2) = W2;
%     elseif cS1 == 0
%         dW1 = n * (R_vec(end,1)-eR1) * (cS2); % This expected reward is stimulus specific, whereas the simulation is usually not
%         dW2 = n * (R_vec(end,2)-eR2) * (cS2);
%         W1 = W1 + transpose(dW1);
%         W2 = W2 + transpose(dW2);
% %         if W2 < 0
% %            W2 = 0;
...
%     if cS1 == 1
%         dW1 = n * (R_vec(end,1)) * (cS1);
%         dW2 = n * (R_vec(end,2)) * (cS1);
%         W1 = W1 + transpose(dW1);
%         W2 = W2 + transpose(dW2);
% %         if W1 < 0
% %             W1 = 0;
