...
%     I = cat(2,I1,I2,I4);
    ...
    [B,FitInfo] = lassoglm(I(1:40,:),Y(1:40),'binomial','CV',10);%,'Alpha',0.1); %performing regression
    ...
    yhat = glmval(wi,I(1:40,:),'logit'); % calcualting predicted behavior
    ws(:,k) = wi; % saving weights to weight history

    
    % VERSION 2 : VANILLA REGRESSION THAT ALLOWS US TO GET ERROR ESTIMATES.
    % NO CROSS VALIDATION
%     B = fitglm(I(1:40,:),Y(1:40),'Distribution','binomial'); % performing regression
%     yhat = glmval(B.Coefficients.Estimate,I(1:40,:),'logit'); % predicted choices
