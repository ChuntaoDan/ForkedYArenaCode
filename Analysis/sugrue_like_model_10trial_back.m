...
%% Choosing behavior with exploration
                ...
                prob_1 = 1/(1 + exp(-beta*(v1(t_num-10) - v2(t_num-10))));
...
%% No behavior being choosen just calculating probability of being correct
                if choice_order(t_num) == 1
                    pred_choice(tau_n,t_num - 10) = prob_1;
                    plotting_choice(tau_n,t_num - 10) = 1-prob_1;
                else
                    pred_choice(tau_n,t_num - 10) = 1-prob_1;
                    plotting_choice(tau_n,t_num - 10) = 1-prob_1;
                end
...
% 
            LL_mat(count,tau_n) = sum(log(pred_choice(tau_n,:)));
            LossLOU_mat(count,tau_n) =mean(pred_choice(tau_n,:));
            typ_p(count,tau_n) = exp(mean(log(pred_choice(tau_n,:))));
        ...
        keyboard
        hold on
        [maxLL,maxLL_tau_n] = max(LL_mat(count,:));
        tau_fig = tau_list(maxLL_tau_n);
        smoothed_pred_choice = [];
        for i = 10:length(choice_order)-1
            smoothed_pred_choice(i-9,:) = mean(plotting_choice(find(tau_list == tau_fig),i-9:i));
        end
...
%         plot([2:240],plotting_choice(tau_fig,:)*90,'LineWidth',4,'Color',[0.75,0,0.75])
        ...
        filename = sprintf('data_sugrueModel_withExp_smoothed_tau_%d.fig',tau_fig);
        savefig(filename)
        keyboard
        close all
