function [nC1,nC2,nI1,nI2,I_list,C_list,v1,v2] = Strategy3_v2(nblocks,ntrials,pR1s,pR2s,window_trials,exp_p)
    ...
    for nt = 1:ntrials
        ...
        %                     v1(nt+1) = mean(I1_v_list);
                        for ct = 1:length(I_list)
                            if I_list(ct) == 1
                                v1_list(length(v1_list)+1) = 1;
                            elseif I_list(ct) == 2
                                v1_list(length(v1_list)+1) = -1;
                            elseif I_list(ct) ==  0
                                if C_list(ct) == 1
                                    v1_list(length(v1_list)+1) = 0;
                                else
                                    v1_list(length(v1_list)+1) = -1;
                                end
                            end 
                        ...
                        v1(((nb-1)*ntrials) + nt+1) = sum(flip(exp_w_list(valid_els)).*v1_list(valid_els));
                    else
        %                     v1(nt+1) = mean(I1_v_list(end-window_trials : end));
                        ...
                        valid_els = find(v1_list ~= -1);      
                         if length(valid_els) < 3
                            v1(((nb-1)*ntrials) + nt+1) = sum(flip(exp_w_list(1:length(valid_els))).*v1_list(valid_els));
                        else    

                            v1(((nb-1)*ntrials) + nt+1) = sum(flip(exp_w_list).*v1_list(valid_els(end-window_trials+1:end)));
                         end
                    end
        ...
        %                     v2(nt+1) = mean(I2_v_list);
                        for ct = 1:length(I_list)
                            if I_list(ct) == 2
                                v2_list(length(v2_list)+1) = 1;
                            elseif I_list(ct) == 1
                                v2_list(length(v2_list)+1) = -1;
                            elseif I_list(ct) ==  0
                                if C_list(ct) == 2
                                    v2_list(length(v2_list)+1) = 0;
                                else
                                    v2_list(length(v2_list)+1) = -1;
                                end
                            end 
                        ...
                        v2(((nb-1)*ntrials) + nt+1) = sum(flip(exp_w_list(valid_els)).*v2_list(valid_els));
        ...
        %                     v2(nt+1) = mean(I2_v_list(end-window_trials : end));
                        ...
                        if length(valid_els) < 3
                            v2(((nb-1)*ntrials) + nt+1) = sum(flip(exp_w_list(1:length(valid_els))).*v2_list(valid_els));
                        else    
                            v2(((nb-1)*ntrials) + nt+1) = sum(flip(exp_w_list).*v2_list (valid_els(end-window_trials+1:end)));
                        end
                    end
