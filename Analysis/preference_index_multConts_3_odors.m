...
function [PI_M,PI_O,cps] = preference_index_multConts_3_odors(air_arm,right_left,x_y_time_color)
        ...
        if mod(choice_count,2) == 0
            ...
            if i == length(air_arm)
                ...
                while no_match == 1
                    ...
                    if sum(x_y_time_color.color(i-jj,:) == M_A_Color )==3 ||sum(x_y_time_color.color(i-jj,:) == M_P_Color )==3
    ...
    PI_O = choices(2)/(choices(2)+choices(1)); % O/(O+P)
    PI_M = choices(3)/(choices(3)+choices(4)); % M/(M+P)
end