...
%         end
        ...
        for i = 1:20
            if i > 5
                error_count_pre_tA(expt_n,cond_n-2,i) = length(find(choice_order(i-5:i,1)== unrewarded_odor))/6;
                error_count_post_tA(expt_n,cond_n-2,i) = length(find(choice_order(i-5:i,4)== unrewarded_odor))/6;
            else
                error_count_pre_tA(expt_n,cond_n-2,i) = length(find(choice_order(1:i,1)== unrewarded_odor))/i;
                error_count_post_tA(expt_n,cond_n-2,i) = length(find(choice_order(1:i,4)== unrewarded_odor))/i;
